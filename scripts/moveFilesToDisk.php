...
// Description
// -----------
// This script exports the raw_content from ciniki_workshops_post_files to files.
//
...
if( $rc['stat'] != 'ok' ) {
    return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.workshops.93', 'msg'=>'Unable to load item', 'err'=>$rc['err']));
}
$tenants = array();
foreach($rc['rows'] as $row) {
    $tenants[$row['id']] = $ciniki['config']['ciniki.core']['storage_dir'] . '/' . $row['uuid'][0] . '/' . $row['uuid'] . '/ciniki.workshops/files';
}
...
foreach($tenants as $tnid => $uuid) {
    ...
    $rc = ciniki_core_dbHashQuery($ciniki, $strsql, 'ciniki.core', 'item');
    if( $rc['stat'] != 'ok' ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.workshops.95', 'msg'=>'Unable to load item', 'err'=>$rc['err']));
    }
    foreach($rc['rows'] as $row) {
