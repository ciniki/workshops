...
function ciniki_workshops_fileUpdate(&$ciniki) {
    ...
    $rc = ciniki_workshops_checkAccess($ciniki, $args['tnid'], 'ciniki.workshops.fileUpdate', 0); 
    if( $rc['stat'] != 'ok' ) { 
        return $rc;
    }
    ...
    if( !isset($rc['file']) ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.workshops.12', 'msg'=>'File does not exist.'));
    }
    $file = $rc['file'];
    ...
    if( isset($args['permalink']) ) {
        ...
        if( $rc['num_rows'] > 0 ) {
            return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.workshops.13', 'msg'=>'You already have a file with this name, please choose another name.'));
        }
    }
    ...
    return ciniki_core_objectUpdate($ciniki, $args['tnid'], 'ciniki.workshops.file', $args['file_id'], $args, 0x07);
}
?>
