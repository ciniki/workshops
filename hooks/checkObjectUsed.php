...
// -------
//
function ciniki_workshops_hooks_checkObjectUsed($ciniki, $business_id, $args) {
	...
	if( $args['object'] == 'ciniki.customers.customer' ) {
		...
		if( isset($rc['num']['items']) && $rc['num']['items'] > 0 ) {
			$used = 'yes';
			$count = $rc['num']['items'];
			$msg .= ($msg!=''?' ':'') . "There " . ($count==1?'is':'are') . " $count workshop registration" . ($count==1?'':'s') . " for this customer.";
		}
	}
